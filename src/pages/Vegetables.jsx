...
const renderStars = (rating) => {
  ...
  while (stars.length < 5) stars.push(<span key={`empty-${stars.length}`} className="text-gray-300">★</span>);
...
const Vegetables = () => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        ...
        const vegetablesItems = res.data.filter(p => p.category === "Vegetables");
        setMongoProducts(vegetablesItems);
      } catch (err) {
  ...
  const allVegetables = mongoProducts;
  

  const filteredVegetables = allVegetables
    .filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()))
    .filter(p => (priceFilter ? p.price <= parseInt(priceFilter) : true));

  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      <h2 className="text-3xl font-bold text-green-800 mb-6 text-center">Fresh Vegetables</h2>
...
// ✅ Dairy.jsx and Grains.jsx are structurally the same, only replace the category.
// I will paste them next if you'd like those full updated versions as well.
