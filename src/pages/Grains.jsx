...
const Grains = () => {
  ...
  useEffect(() => {
    const fetchGrains = async () => {
      try {
        ...
        const grainsItems = res.data.filter(p => p.category === "Grains");
        setMongoProducts(grainsItems);
      } catch (err) {
        console.error("Failed to fetch grains:", err);
      }
    };
  ...
  const allGrains = mongoProducts;

  const filteredGrains = allGrains
    .filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()))
    .filter(p => (priceFilter ? p.price <= parseInt(priceFilter) : true));

  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      <h2 className="text-3xl font-bold text-green-800 mb-6 text-center">Grains & Cereals</h2>
