...
const Dairy = () => {
  ...
  useEffect(() => {
    const fetchDairy = async () => {
      try {
        ...
        const dairyItems = res.data.filter(p => p.category === "Dairy");
        setMongoProducts(dairyItems);
      } catch (err) {
        console.error("Failed to fetch dairy:", err);
      }
    };
  ...
  const allDairy = mongoProducts;

  const filteredDairy = allDairy
    .filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()))
    .filter(p => (priceFilter ? p.price <= parseInt(priceFilter) : true));

  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      <h2 className="text-3xl font-bold text-green-800 mb-6 text-center">Dairy Products</h2>
